...
typedef struct VS {
	void (*shader)(void *p_vertex_input_data, void *p_vertex_output_data, u32 vertex_id);
...
inline bool is_inside_edge(const EdgeFunction *p_edge_func, v2i32 frag_coord) {
	i32 a = p_edge_func->a;
	i32 b = p_edge_func->b;
	i32 c = p_edge_func->c;
	i32 signed_dist = ((a * frag_coord.x) << NUM_SUB_PIXEL_PRECISION_BITS) + ((b * frag_coord.y) << NUM_SUB_PIXEL_PRECISION_BITS) + c;
...
inline bool is_inside_triangle(const EdgeFunction *p_edge_func, v2i32 frag_coord) {
	bool w0 = is_inside_edge(p_edge_func, frag_coord);
	bool w1 = is_inside_edge(p_edge_func + 1, frag_coord);
	bool w2 = is_inside_edge(p_edge_func + 2, frag_coord);
	return w0 && w1 && w2;
}

inline v2f32 compute_barycentric_coords() {
...
inline void interpolate_attribute(v4f32 *p_triangle_vertex_data, v2f32 barycentric_coords, u8 num_attributes_per_vertex, u8 attribute_index) {
	...
	*p_v0_attribute = v4f32_add_v4f32(*p_v0_attribute, v4f32_add_v4f32(v4f32_mul_f32(v1_attribute, barycentric_coords.x), v4f32_mul_f32(v2_attribute, barycentric_coords.y)));
	return;
}

typedef struct TriangleSetup {
	EdgeFunction a_edge_funtions[3];
	f32 one_over_area;
};

SuprematistVertex suprematist_vertex_buffer[] = {
	{ { 0.31219, 0.15  }, { 0.08627, 0.07450, 0.07058 } },
	{ { 0.92804, 0.142 }, { 0.08627, 0.07450, 0.07058 } },
	{ { 0.92682, 0.883 }, { 0.08627, 0.07450, 0.07058 } },
	{ { 0.32195, 0.889 }, { 0.08627, 0.07450, 0.07058 } },
	{ { 0.07317, 0.41  }, { 0.17647, 0.17254, 0.32549 } },
	{ { 0.66463, 0.614 }, { 0.17647, 0.17254, 0.32549 } },
	{ { 0.08170, 0.887 }, { 0.17647, 0.17254, 0.32549 } },
};
...
void draw_indexed(UINT index_count /* TODO(cerlet): Use UINT start_index_location, int base_vertex_location*/) {
	...
	for(u32 vertex_id = 0; vertex_id < vertex_count; ++vertex_id) {
		graphics_pipeline.vs.shader(p_vertex_input_data, p_vertex_output_data, vertex_id);
	}
	...
	for(u32 triangle_index = 0; triangle_index < triangle_count; ++triangle_index) {
		...
		set_edge_function(&a_edge_functions[2], signed_area, x[0], y[0], x[1], y[1]);
		set_edge_function(&a_edge_functions[0], signed_area, x[1], y[1], x[2], y[2]);
		set_edge_function(&a_edge_functions[1], signed_area, x[2], y[2], x[0], y[0]);

		f32 one_over_area = fabs(1.f / (f32)(signed_area >> (NUM_SUB_PIXEL_PRECISION_BITS * 2)));
		...
		// triangle culling
		{
			...
			for(i32 y = min_bounds.y; y <= max_bounds.y; ++y) {
				for(i32 x = min_bounds.x; x<max_bounds.x; ++x) {
					...
					if(is_inside_triangle(a_edge_functions, frag_coords)) { // use edge functions for inclusion testing
						
						v2f32 barycentric_coords = compute_barycentric_coords();
...
void render() {
	graphics_pipeline.ia.input_layout = sizeof(pass_through_vs.vs_input);
	graphics_pipeline.ia.primitive_topology = PRIMITIVE_TOPOLOGY_TRIANGLELIST;
	graphics_pipeline.ia.p_index_buffer = suprematist_index_buffer;
	graphics_pipeline.ia.p_vertex_buffer = suprematist_vertex_buffer;

	graphics_pipeline.vs.output_register_count = sizeof(pass_through_vs.vs_output) / sizeof(v4f32);
	graphics_pipeline.vs.shader = pass_through_vs.vs_main;

	Viewport viewport = { 0.f,0.f,(f32)frame_width,(f32)frame_height,0.f,1.f };
	graphics_pipeline.rs.viewport = viewport;

	graphics_pipeline.ps.shader = pass_through_ps.ps_main;
